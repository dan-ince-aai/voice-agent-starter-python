...
`AGENT_ID` decides create versus update. Unset, `publish_agent` sends `POST /v1/agents` and writes the returned id to `.env`. Set, it sends `PUT /v1/agents/{id}`, falling back to a create if that returns 404.
